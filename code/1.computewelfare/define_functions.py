...
def compute_linkdists(bl):
    return [bdrive[bl[i]][bl[i+1]] for i in range((len(bl)-1))]

# construct travel times between connected cells (add linkdists later)
def compute_busline(bl, linkdists = compute_linkdists(bl)):
    dto1 = [sum(linkdists[0:i]) for i in range(len(bl))]
    stops = np.asarray([np.abs(np.subtract(dto1, dto1[i])) for i in range(len(bl))])
    return stops
...
def compute_direct_connections_route(bl):
    ...
    C[np.array(bl)[:,np.newaxis], bl] = compute_busline(bl)
